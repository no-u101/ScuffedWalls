...
namespace ScuffedWalls.Functions
{
    ...
    class GuitarHerofy : SFunction
    {
        public override void Run()
        {
            ...
            bool stackToSlides = GetParam("stacktoslides", false, p => bool.Parse(p));
            ...
            bool forceParity = GetParam("forceparity", false, p => bool.Parse(p));
            float parityViewDist = GetParam("parityviewdist", 4, p => float.Parse(p));

            bool onlyConvert = GetParam("onlynotes", false, p => bool.Parse(p));
            bool onlyModels = GetParam("onlymodels", false, p => bool.Parse(p));
            ...
            if (!onlyModels)
            {
                ...
                int lastNoteDirectionR = 1;
                int lastNoteDirectionL = 1;
                ...
                foreach (var note in InstanceWorkspace.Notes.OrderBy(o => (float)o._time))
                {
                    if (note._time.toFloat() >= starttime && note._time.toFloat() <= endtime && (note._type.toFloat() == 1 || note._type.toFloat() == 0))
                    {
                        ...
                        if (noteType == 0) //Left
                        {
                            ...
                            //Directions
                            lastDirInst = lastNoteDirectionL;
                        ...
                        else if (noteType == 1) //right
                        {
                            ...
                            //Directions
                            lastDirInst = lastNoteDirectionR;
                        }
                        ...
                        //Stacks / windows to side slider
                        if (stackToSlides && note._time.toFloat() == lastNoteInst._time.toFloat() && note._lineIndex.toFloat() != lastNoteInst._lineIndex.toFloat())
                        {
                            float dir = note._lineIndex.toFloat() - lastNoteInst._lineIndex.toFloat();
                            if (dir < 0)
                            {
                                //Going to the left
                                note._cutDirection = 2;
                                lastNoteInst._cutDirection = 2;
                            } else
                            {
                                note._cutDirection = 3;
                                lastNoteInst._cutDirection = 3;
                            }

                            ScuffedLogger.Default.Log(note._time.toFloat().ToString());
                            goto NoteAllowed;
                        }
                    ...
                    NoteAllowed:
                        ...
                        {
                            lastNoteL = note; 
                            lastNoteDirectionL = lastDirInst;
                        }
                        ...
                        {
                            lastNoteR = note; 
                            lastNoteDirectionR = lastDirInst;
                        }
            ...
            if (!onlyConvert)
            {
                ...
                foreach (var note in InstanceWorkspace.Notes.OrderBy(o => o._time.toFloat()))
                {
                    if (note._time.toFloat() >= starttime && note._time.toFloat() <= endtime && (note._type.toFloat() == 1 || note._type.toFloat() == 0))
                    {
                        ...
                        if ((note._cutDirection.toFloat() == 2 || note._cutDirection.toFloat() == 3))
                            modelType += 4;
                        ...
                        if (Convert.ToInt32(note._type.ToString()) == 0 || Convert.ToInt32(note._type.ToString()) == 1)
                        {
                            ...
                            if (modelType == 4)
                                Delta.RotationEul = new Vector3(0, 180, 0);
